...
public class UserDao {
	...
	*/
	...
	public User select(Connection connection, String accountOrEmail, String password) {
		...
		try {
			//usersテーブルの該当のaccountまたはemailとpasswordのデータを参照（？はパラメータ変数）
			String sql = "SELECT * FROM users WHERE (account = ? OR email = ?) AND password = ?";

			//SQL文をDBに送るための準備（詳細不明）
			ps = connection.prepareStatement(sql);

			//sqlに要素を渡す（第一引数はパラメータ変数）
			...
			//usersが空の場合はUserServiceにnullを返す
			if (users.isEmpty()) {
				return null;
				//usersの要素が2つ以上ある場合はUserServiceにエラーメッセージを返す
